			...
			while self.getTweets:
				try:
					...
					if maxid == 0 and self.totalTweets == 0:
						...
						tweets = twitterapi.statuses.user_timeline(id = handle_id,count = 1000,include_rts = False,tweet_mode = "extended")
					else:
						if self.totalTweets != 0:
							page += 1
							print 'Continuing to next page for handle ',str(self.handle.pk),' page ',page
							tweets = twitterapi.statuses.user_timeline(id = handle_id,count = 1000,include_rts = False,max_id = (maxid-1),tweet_mode = "extended")
						else:
							print "Resuming saving tweets from ",maxid
							tweets = twitterapi.statuses.user_timeline(id = handle_id,count = 1000,include_rts = False,since_id = (maxid-1),tweet_mode = "extended")
					self.requests += 1
